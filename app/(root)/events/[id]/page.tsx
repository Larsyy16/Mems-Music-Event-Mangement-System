...
<div className="bg-gray-5 p-8 rounded-lg shadow-md">
      ...
      <Image src="/assets/icons/clock.svg" width={24} height={24} alt="location" className="mr-2" />
        <div>
        <p className="text-gray-700 inline-block">Start Time: {new Date(event.startDateTime).toLocaleString()}</p>
        <p className="text-gray-700 ">End Time: {new Date(event.endDateTime).toLocaleString()}</p>
        </div>
      </div>
    ...
    <CardGroup data={similarEvents?.data} emptyTitle='No events found.'
